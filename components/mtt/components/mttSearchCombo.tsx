...
export function MttSearchCombo({
  placeholder,
  className,
  SelectValues,
  Onselect,
  returnAValue,
  InitialValue, // New prop for initial selection
}: {
  placeholder: string
  className?: string
  SelectValues: SelectValues[]
  Onselect: (value: string) => void
  returnAValue?: boolean
  InitialValue?: string | null // Can be a string or null
}) {
  const [open, setOpen] = React.useState(false)
  const [selectedId, setSelectedId] = React.useState<string | null>(
    InitialValue ?? null // Initialize with InitialValue if provided
  )
  const [searchQuery, setSearchQuery] = React.useState<string>("")

  const frameworks = SelectValues
  const selectedLabel =
    frameworks.find((framework) => framework.id === selectedId)?.label || placeholder
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger className="data-[state=selected]:!bg-red-700" asChild>
        <Button
          ...
          aria-expanded={open}
          className={cn(
            "w-full text-[13px] font-normal justify-between mtt-BaseShadeHover5White group text-textPri rounded-md h-InputHeight !border-input ",
            className
          )}
        >
          {selectedLabel}

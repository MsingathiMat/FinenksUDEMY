...
const OriginalForm = ({ Utilities }: { Utilities: UtilitiesProp }) => {
  ...
  const FormMethods = useForm<FormType>({
    defaultValues: {
      ...
      Amount: 0.0,
      ItemStatus: "ACTIVE",
      UserId: userData?.activeId || "", // Default UserId
    },
  ...
  // Ensure Quantity defaults for specific ItemType
  useEffect(() => {
    FormMethods.setValue("Quantity", -1);
  }, [ItemType]);

  // Ensure UserId is updated when userData changes
  useEffect(() => {
    if (userData) {
      FormMethods.setValue("UserId", userData.activeId);
    }
  ...
  // Prevent submission if UserId is not set
  const isFormDisabled = !!FormMethods.watch("UserId");
      ...
      >
        <div className="mtt-center gap-6 mt-2 !flex-row w-fit">
          <div className="mtt-center gap-4 !flex-col">
            ...
            <MttSelect readOnly={readOnly} name="ItemStatus" label="Select Status"Options={[{ value: "ACTIVE", label: "Active" },{ value: "INACTIVE", label: "Inactive" },]}
            />
            ...
            <IsLoading className="w-full mtt-center" isLoading={FormIsLoading}>
              ...
              {isFormDisabled?<MttSubmit >Add</MttSubmit>:"UserId not initialized"}
